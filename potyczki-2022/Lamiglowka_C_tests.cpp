...
void move_by(vec<string>& rows, char move) {
    switch (move) {
        case 'L':
            move_left(rows);
            break;
...
int main(void) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        int idx = positions[new_rotation / N][new_pos.first][new_pos.second];
        if (idx == -1) {
            poses.push_back(new_pos);
            rotations.push_back(new_rotation);

            positions[new_rotation / N][new_pos.first][new_pos.second] =
                poses.size() - 1;

            moves += move;
        } else {
            while (poses.size() > idx + 1) {
                positions[rotations.back() / N][poses.back().first]
                         [poses.back().second] = -1;
                poses.pop_back();
                rotations.pop_back();
            }
    ...
    for (char move : all_moves) {
        move_by(copy_rows, move);
    }

    // print_rows(rows);

    cout << (rows == copy_rows) << endl;
}