...
struct Edge {
    ...
    int income;
    Edge() = default;
    Edge(int to, int income) : to(to), income(income) {}
};
struct Node : public vec<Edge> {};
...
void solution() {
    ...
    loop(i, n - 1) {
        ...
        v--;
        e--;
        graph[v].emplace_back(e, inc);
        graph[e].emplace_back(v, inc);
    }
    int idx = 0;
    cout << "graph {\n";
    for (auto& node : graph) {
        for (auto& edge : node) {
            ...
            cout << '\t' << idx << "--" << edge.to << "[label= " << edge.income
                 << "]\n";
        }
        idx++;
    }
